...
class VisionMambaDet(VisionMamba, Backbone):
    ...
    def init_weights(self, pretrained=None):
        ...
        if isinstance(pretrained, str):
            ...
            # support both {"model": ...} format dan flat state dict
            state_dict_model = state_dict.get("model", state_dict)
            # pop classification head
            ...
            proj_w = state_dict_model.get("patch_embed.proj.weight")
            if proj_w is not None and self.patch_embed.patch_size[-1] != proj_w.shape[-1]:
